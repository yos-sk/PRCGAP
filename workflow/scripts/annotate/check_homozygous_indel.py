...
class SNVinfo:
    ...
    def insert(self, info):
        ...
        self.vaf = float(info[15])
        self.depth = int(info[16])
        self.normal_vaf = float(info[17])
        self.normal_depth = int(info[18])
        self.mutation_id = info[19]
        self.haplotype = info[20]
        self.kmer_ratio = float(info[21])
        self.liftoff_gene = info[22]
        self.liftoff_info = info[23]
        self.cgc = info[24]
        self.cmrg = info[25]
        self.rmsk = info[26]
        self.rmsk_type = info[27]
        self.contig_size = int(info[28])
        self.misassembly = info[29]
        self.centromere = info[30]
        self.segdup = info[31]
        self.segdup_similarity = info[32]
        self.other = info[33]
    ...
    def output(self):
        print(self.contig, self.start, self.end, self.ref, self.alt, self.GRCh38_contig, self.GRCh38_pos, self.GRCh38_ref, self.GRCh38_alt, self.GRCh38_status,
              self.chm13_contig, self.chm13_pos, self.chm13_ref, self.chm13_alt, self.chm13_status, self.vaf, self.depth, self.normal_vaf, self.normal_depth, self.mutation_id, self.haplotype,
