...
RESOURCE_MAPPING = {
    ...
    "slots"            : ("slots",),
    # NCC SGE site uses s_vmem (per-slot) for memory requests, so map
    # snakemake's mem_mb -> s_vmem here. The total mem_mb is divided by
    # threads below before submission so the per-slot s_vmem matches the
    # snakemake-standard "total job memory" semantics.
    "s_vmem"           : ("s_vmem", "mem_mb", "mem", "memory", "soft_memory", "soft_virtual_memory"),
    "h_vmem"           : ("h_vmem", "virtual_memory"),
...
def format_job_properties(string):
    ...
    # snakemake option. `threads` is needed so cluster.yaml entries like
    # `pe: "def_slot {threads}"` can pick up the per-rule thread count.
    rulename = job_properties.get('rule') or job_properties.get('groupid') or 'snakejob'
...
#
# NCC SGE: s_vmem is per-slot, but snakemake's mem_mb is total job memory.
# Convert to per-slot by dividing by threads (round up) before passing in.
snake_resources = dict(job_properties.get("resources", {}))
snake_threads = int(job_properties.get("threads", 1) or 1)
if "mem_mb" in snake_resources and snake_threads > 1:
    snake_resources["mem_mb"] = max(1, math.ceil(int(snake_resources["mem_mb"]) / snake_threads))
update_double_dict(qsub_settings, parse_qsub_settings(snake_resources, option_mapping={}))
