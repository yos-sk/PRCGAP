...
def main():
    ...
    hout = open(output_prefix + ".nanomonsv.supporting_read.sorted.txt", 'w')
    subprocess.check_call(["sort", "-k9,9", "-k10,10n", support_read_file], stdout = hout)
    hout.close()
    
    filter_support_reads(nanomonsv_results, output_prefix + ".nanomonsv.supporting_read.sorted.txt", output_prefix + ".nanomonsv.supporting_read.sorted.filtered.txt")
    get_edge(output_prefix + ".nanomonsv.supporting_read.sorted.filtered.txt", output_prefix + ".nanomonsv.edge.txt")
    connect(output_prefix + ".nanomonsv.edge.txt", output_prefix + ".nanomonsv.connect.txt")
